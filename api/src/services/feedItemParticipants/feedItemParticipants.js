...
let table = 'feedItemParticipant'

export const createFeedItemParticipant = async ({ input }) => {
  try {
...
export const feedItemParticipants = async ({
  filter,
  skip,
  orderBy,
  q,
  ...
  try {
    ...
    let _take = (() => {
      let limit =
        take ||
        parseInt(preferences['feeditemparticipant.pageSize'], 10) ||
...
export const feedItemParticipant = async ({ id }) => {
  try {
...
export const updateFeedItemParticipant = async ({ id, input }) => {
  try {
...
export const deleteFeedItemParticipant = async ({ id }) => {
  try {
...
export const FeedItemParticipant = {
  feedItem: (_obj, { root }) =>
    db[table].findUnique({ where: { id: root.id } }).feedItem(),
  participant: (_obj, { root }) =>
    db[table].findUnique({ where: { id: root.id } }).participant(),
}
