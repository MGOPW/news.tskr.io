...
export const createParticipant = async ({ input }) => {
  try {
...
export const participants = async ({ filter, skip, orderBy, q, take }) => {
  try {
    let preferences = context.currentUser.preferences
    let _take = (() => {
      let limit =
        take ||
        parseInt(preferences['participant.pageSize'], 10) ||
...
export const participant = async ({ id }) => {
  try {
...
export const updateParticipant = async ({ id, input }) => {
  try {
...
export const Participant = {
  FeedItemParticipant: (_obj, { root }) =>
    db[table].findUnique({ where: { id: root.id } }).FeedItemParticipant(),
}
