...
export const Failure = ({ error }) => (
  <div className="rw-cell-error">{error.participant}</div>
)
...
export const Success = ({
  ...
  let [data, setData] = useState(participants)
  ...
  return (
    <Box p={3} backgroundColor={'white'}>
      <Heading>Participants ({data.count})</Heading>
      <Flex>
        <Box>
          {participants.q !== null && (
            <Button
        ...
        <Button
          ...
          variant="solid"
          onClick={() => {
            navigate(routes.newParticipant())
          }}
        >
          New participant
        </Button>
      ...
      <TableQuery
        ...
        rawQuery={participants.q}
        inputPlaceholder="Search"
        link={(query) => {
          return routes.participants({ q: query })
        }}
        setSkip={setSkip}
      />
      ...
      <Table variant="striped" colorScheme={'green'} size="xs">
        ...
        <TableRows
          ...
          data={data}
          model="participants"
          deleteMutation={DELETE_PARTICIPANT_MUTATION}
