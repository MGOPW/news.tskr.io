...
export default async ({ args }) => {
  ...
  let getArticles = async (start) => {
    ...
    let url = `https://community.servicenow.com/api/sn_communities/v1/community/contents?last=${end}&stFrom=${start}&before=${now}&forum=&type=5eaa334a5f10030069c587dc3f73130b&sort=created&filters=undefined`
    ...
    for (let article of jsonResults.result.contents) {
      if (article.userAvatarObject.name !== "tracyo'brien") {
        let transformedItem = {
          url: `https://community.servicenow.com/community?id=community_article&sys_id=${article.sys_id}`,
          title: article.title,
